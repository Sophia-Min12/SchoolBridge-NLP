"""
evaluate_compare_v3_2_20260509.py
====================================
담당: 경이 (kyeongyi)
작성일: 2026-05-09

목적:
    split_v3_1_20260509.csv (15948행) 기준으로
    두 모델의 성능을 비교 저장 출력한다.

    [비교 모델]
      1. Simple   : TF-IDF + Logistic Regression (글로벌 베이스라인)
      2. KcELECTRA: 파인튜닝 모델 v3_2 — beomi/kcelectra-base (진짜 KcELECTRA)
                   (12_train_kcelectra_v3_2_20260509.ipynb 실행 후)

    [v3_1과의 차이]
      v3_1은 monologg/koelectra-base-v3-discriminator (KoELECTRA)를 사용.
      v3_2는 beomi/kcelectra-base (KcELECTRA — 진짜)를 사용.
      Simple 베이스라인은 동일한 데이터(split_v3_1_20260509.csv)로 평가.

    [공정 비교 원칙]
      - 두 모델 모두 split_v3_1_20260509.csv의 동일한 test 세트로 평가
      - Simple은 동일한 train 세트(v3_1 데이터)로 학습
      - KcELECTRA v3_2 JSON이 없으면 checkpoints/kcelectra-category-v3_2/에서 직접 추론

    [출력 파일 - data/20260509/ 폴더]
      eval_results_simple_v3_2_20260509.json
      eval_results_kcelectra_v3_2_20260509.json
      eval_comparison_summary_v3_2_20260509.csv

실행:
    cd model/classification
    python scripts/evaluate_compare_v3_2_20260509.py
    python scripts/evaluate_compare_v3_2_20260509.py --split val
    python scripts/evaluate_compare_v3_2_20260509.py --retrain   # Simple 강제 재학습
"""
...
SIMPLE_PKL     = _BASE / "checkpoints" / "simple_tfidf_logreg_v3_2_20260509.pkl"
KCELECTRA_CKPT = _BASE / "checkpoints" / "kcelectra-category-v3_2"
OUT_DIR        = _BASE / "data" / "20260509"
TS             = "v3_2_20260509"

LABELS = ["일정", "준비물", "제출", "비용", "건강·안전", "기타"]

# Simple 베이스라인 (v3_1 데이터 기준, evaluate_compare_v3_1_20260509.py 결과)
SIMPLE_BASELINE_F1 = 0.7590
KC_V3_1_F1         = None   # v3_1(KoELECTRA) 결과 — Colab 학습 후 채워질 값
...
def _load_simple() -> Pipeline:
    # v3_1 pkl도 재활용 가능 (같은 데이터/설정)
    v3_1_pkl = _BASE / "checkpoints" / "simple_tfidf_logreg_v3_1_20260509.pkl"
    if SIMPLE_PKL.exists():
        with open(SIMPLE_PKL, "rb") as f:
            return pickle.load(f)
    if v3_1_pkl.exists():
        print(f"[simple] v3_1 PKL 재활용 (같은 데이터): {v3_1_pkl.name}")
        return pickle.load(open(v3_1_pkl, "rb"))
    return train_simple()
...
# -----------------------------------------------------------------------
# KcELECTRA v3_2 (beomi/kcelectra-base 파인튜닝)
# -----------------------------------------------------------------------
...
def evaluate_kcelectra(split: str = "test") -> dict:
    """
    KcELECTRA v3_2 평가.
    eval_results_kcelectra_v3_2_20260509.json이 있으면 재활용 (Colab 결과 복붙 시).
    """
    ...
    if not _kcelectra_ready():
        ...
        print("  12_train_kcelectra_v3_2_20260509.ipynb (Colab) 실행 후")
        print("  kcelectra-category-v3_2/ 를 checkpoints/ 에 배치하세요.")
        return {}
    ...
    print("\n[KcELECTRA v3_2 — beomi/kcelectra-base] 분류 리포트")
    print(classification_report(true_labels, pred_labels, labels=LABELS, zero_division=0))
...
def save_and_compare(simple_res: dict, kcelectra_res: dict) -> None:
    ...
    rows = [{"model": "Simple (TF-IDF + LR, v3_1 데이터)", **_summary_row(simple_res)}]
    if kcelectra_res:
        rows.append({"model": "KcELECTRA v3_2 (beomi/kcelectra-base, fine-tuned)", **_summary_row(kcelectra_res)})
    ...
    print("  KcELECTRA v3_2: beomi/kcelectra-base (진짜 KcELECTRA)")
    print("=" * 65)
    print(f"  Simple 기준값 (v3_1 데이터): {SIMPLE_BASELINE_F1:.4f}")
    print(f"  Simple v3_2  Macro F1 : {simple_res['macro_f1']:.4f}"
    ...
    if kcelectra_res:
        ...
        print(f"  KcELECTRA v3_2 Macro F1 : {kcelectra_res['macro_f1']:.4f}")
        print(f"  Simple v3_2 대비         : {delta_vs_simple:+.4f}")
        print(f"  Simple 기준값 대비       : {delta_vs_base:+.4f}")
        print()

        if delta_vs_base >= 0.05:
            print("  => KcELECTRA v3_2이 Simple 기준값 대비 5%+ 향상 -- 목표 달성!")
        elif delta_vs_simple > 0:
            print("  => KcELECTRA v3_2이 Simple v3_2보다 향상됨")
        else:
            print("  => Simple v3_2이 더 높음 -- 재학습 필요")
        ...
        if all_better:
            print("  => 전 카테고리에서 KcELECTRA v3_2 > Simple (목표 달성!)")
        else:
            worse = [
        ...
        print("  KcELECTRA v3_2: 평가 미완료")
        print("  12_train_kcelectra_v3_2_20260509.ipynb (Colab) 실행 후")
        print("  eval_results_kcelectra_v3_2_20260509.json을 data/20260509/ 에 배치하세요.")
...
def main() -> None:
    ...
    print(f"KcELECTRA 모델: beomi/kcelectra-base (v3_2 — 진짜 KcELECTRA)")
