"""
evaluate_compare_v2_2_20260509.py
====================================
담당: 경이 (kyeongyi)
작성일: 2026-05-09

목적:
    split_v2_20260504.csv (695개) 기준으로
    두 모델의 성능을 비교 저장 출력한다.

    [비교 모델]
      1. Simple   : TF-IDF + Logistic Regression (베이스라인)
      2. KcELECTRA: 파인튜닝 모델 v2_2 — beomi/kcelectra-base (진짜 KcELECTRA)
                   (14_train_kcelectra_v2_2_20260509.ipynb 실행 후)

    [v2와의 차이]
      v2는 monologg/koelectra-small-v3-discriminator (KoELECTRA-small)를 사용.
      v2_2는 beomi/kcelectra-base (KcELECTRA — 진짜)를 사용.
      Simple 베이스라인은 동일한 데이터(split_v2_20260504.csv)로 평가.

    [공정 비교 원칙]
      - 두 모델 모두 split_v2_20260504.csv의 동일한 test 세트(69개)로 평가
      - Simple은 동일한 train 세트(556개)로 학습
      - KcELECTRA v2_2 JSON이 없으면 checkpoints/kcelectra-category-v2_2/에서 직접 추론

    [출력 파일 - data/20260509/ 폴더]
      eval_results_simple_v2_2_20260509.json
      eval_results_kcelectra_v2_2_20260509.json
      eval_comparison_summary_v2_2_20260509.csv

실행:
    cd model/classification
    python scripts/evaluate_compare_v2_2_20260509.py
    python scripts/evaluate_compare_v2_2_20260509.py --split val
    python scripts/evaluate_compare_v2_2_20260509.py --retrain   # Simple 강제 재학습
"""
...
SPLIT_CSV      = _BASE / "data" / "20260504" / "split_v2_20260504.csv"
SIMPLE_PKL     = _BASE / "checkpoints" / "simple_tfidf_logreg_v2_2_20260509.pkl"
KCELECTRA_CKPT = _BASE / "checkpoints" / "kcelectra-category-v2_2"
OUT_DIR        = _BASE / "data" / "20260509"
TS             = "v2_2_20260509"

LABELS = ["일정", "준비물", "제출", "비용", "건강·안전", "기타"]

# Simple 베이스라인 (v2 데이터 기준, evaluate_compare_v2_20260504.py 결과)
...
def _load_simple() -> Pipeline:
    # v2 pkl도 재활용 가능 (같은 데이터/설정)
    v2_pkl = _BASE / "checkpoints" / "simple_tfidf_logreg_v2_20260504.pkl"
    if SIMPLE_PKL.exists():
        with open(SIMPLE_PKL, "rb") as f:
            return pickle.load(f)
    if v2_pkl.exists():
        print(f"[simple] v2 PKL 재활용 (같은 데이터): {v2_pkl.name}")
        return pickle.load(open(v2_pkl, "rb"))
    return train_simple()
...
# -----------------------------------------------------------------------
# KcELECTRA v2_2 (beomi/kcelectra-base 파인튜닝)
# -----------------------------------------------------------------------
...
def evaluate_kcelectra(split: str = "test") -> dict:
    """
    KcELECTRA v2_2 평가.
    eval_results_kcelectra_v2_2_20260509.json이 있으면 재활용 (Colab 결과 복붙 시).
    """
    ...
    if not _kcelectra_ready():
        ...
        print("  14_train_kcelectra_v2_2_20260509.ipynb (Colab) 실행 후")
        print("  kcelectra-category-v2_2/ 를 checkpoints/ 에 배치하세요.")
        return {}
    ...
    print("\n[KcELECTRA v2_2 — beomi/kcelectra-base] 분류 리포트")
    print(classification_report(true_labels, pred_labels, labels=LABELS, zero_division=0))
...
def save_and_compare(simple_res: dict, kcelectra_res: dict) -> None:
    ...
    rows = [{"model": "Simple (TF-IDF + LR, v2 데이터)", **_summary_row(simple_res)}]
    if kcelectra_res:
        rows.append({"model": "KcELECTRA v2_2 (beomi/kcelectra-base, fine-tuned)", **_summary_row(kcelectra_res)})
    ...
    print("\n" + "=" * 65)
    print("  성능 비교 결과 (v2 데이터 - 695개)")
    print("  KcELECTRA v2_2: beomi/kcelectra-base (진짜 KcELECTRA)")
    print("=" * 65)
    print(f"  Simple 기준값 (v2 데이터): {SIMPLE_BASELINE_F1:.4f}")
    print(f"  Simple v2_2  Macro F1 : {simple_res['macro_f1']:.4f}"
    ...
    if kcelectra_res:
        ...
        print(f"  KcELECTRA v2_2 Macro F1 : {kcelectra_res['macro_f1']:.4f}")
        print(f"  Simple v2_2 대비         : {delta_vs_simple:+.4f}")
        print(f"  Simple 기준값 대비       : {delta_vs_base:+.4f}")
        print()

        if delta_vs_base >= 0.05:
            print("  => KcELECTRA v2_2이 Simple 기준값 대비 5%+ 향상 -- 목표 달성!")
        elif delta_vs_simple > 0:
            print("  => KcELECTRA v2_2이 Simple v2_2보다 향상됨")
        else:
            print("  => Simple v2_2이 더 높음 -- 재학습 필요")
        ...
        if all_better:
            print("  => 전 카테고리에서 KcELECTRA v2_2 > Simple (목표 달성!)")
        else:
            worse = [
        ...
        print("  KcELECTRA v2_2: 평가 미완료")
        print("  14_train_kcelectra_v2_2_20260509.ipynb (Colab) 실행 후")
        print("  eval_results_kcelectra_v2_2_20260509.json을 data/20260509/ 에 배치하세요.")
...
def main() -> None:
    ...
    print(f"평가 시작 - split: {args.split}, 데이터: v2_20260504 (695개)")
    print(f"KcELECTRA 모델: beomi/kcelectra-base (v2_2 — 진짜 KcELECTRA)")
